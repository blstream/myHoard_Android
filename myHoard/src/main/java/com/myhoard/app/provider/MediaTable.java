...
public class MediaTable extends DatabaseTable {
	...
	private static final String DEFAULT_SORT_ORDER = Media._ID + " ASC";

	public MediaTable(Context context, int code) {
		super(context, code, Media.TABLE_NAME);
	}
	...
	public Cursor query(SQLiteDatabase db, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		c.setNotificationUri(getContext().getContentResolver(), Media.CONTENT_URI);
		return c;
	...
	public Uri insert(SQLiteDatabase db, ContentValues initialValues) {
		...
		if (rowId > 0) {
			...
			Uri uri = ContentUris.withAppendedId(Media.CONTENT_URI, rowId);
		...
		throw new SQLException("Failed to insert row into " + Media.CONTENT_URI);
	}
}
